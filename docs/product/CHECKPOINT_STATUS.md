...
| 17 | IN_PROGRESS | Collaboration, sharing, and presentation — single-owner surface complete and correctly session-scoped (comments, approvals with atomic decide-guard, tokenized read-only share links with expiry/revocation, role permissions, presentation mode) and fully tested (29 tests). Remaining: true multi-user collaboration (external collaborators authoring under their own identity + enforced non-owner roles) |
| 18 | IN_PROGRESS | Grounded architecture copilot — keyword-overlap grounding answers real natural-language questions (was substring-only, near-non-functional), now grounds component, finding, relationship/connection (edge citations), category-inventory, AND cost ("what does this cost?" → loaded estimate) and capacity/failure ("what fails first under load?" → simulation) questions, each with its own citation, confidence + refusal; the Copilot workspace loads cost + simulation and passes them as grounding context (tested). Remaining: migration-mapping grounding + live AI-provider wiring |

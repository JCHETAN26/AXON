...
export class TelemetryService {
  ...
  async listTelemetrySources(projectId: string) {
    return this.db
      ...
      .where(
        and(
          eq(telemetrySources.projectId, projectId),
          eq(telemetrySources.ownerId, this.ownerId)
        )
      )
  ...
  async getCalibratedCapacityProfile(projectId: string) {
    ...
    const mockSamples: TelemetryMetricSample[] = [
      {
    ...
    return calibrateCapacityFromTelemetry(mockSamples);
  }
}
